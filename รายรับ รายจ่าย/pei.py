...
class income_outcome(tk.Tk):
    def __init__(x):
        ...
        x.รายได้ = 0.0
        x.ค่าใช้จ่าย = 0.0
        x.ประวัติรายได้ = []  # เพิ่มลิสต์เพื่อเก็บประวัติรายได้
        x.ประวัติค่าใช้จ่าย = []  # เพิ่มลิสต์เพื่อเก็บประวัติค่าใช้จ่าย
        x.หน้าต่าง()
    ...
    def เพิ่มรายได้(x):
        ...
        x.ประวัติรายได้.append(จำนวน)  # เก็บประวัติรายได้
    ...
    def เพิ่มค่าใช้จ่าย(x):
        ...
        x.ประวัติค่าใช้จ่าย.append(จำนวน)  # เก็บประวัติค่าใช้จ่าย

    def แสดงสรุป(x):
        รายได้สุทธิ = x.รายได้ - x.ค่าใช้จ่าย
        x.รายได้สุทธิ_label.config(text=f"รายได้สุทธิ: {รายได้สุทธิ} บาท")
        x.แสดงประวัติ()  # เรียกใช้เมธอดแสดงประวัติ

    def แสดงประวัติ(x):
        # สร้างหน้าต่างใหม่เพื่อแสดงประวัติ
        x.history_window = tk.Toplevel(x)
        x.history_window.title("ประวัติรายได้และค่าใช้จ่าย")

        # สร้างและแสดงประวัติรายได้
        income_label = tk.Label(x.history_window, text="ประวัติรายได้")
        income_label.pack()
        for income in x.ประวัติรายได้:
            income_item = tk.Label(x.history_window, text=f"รายได้: {income} บาท")
            income_item.pack()

        # สร้างและแสดงประวัติค่าใช้จ่าย
        expenses_label = tk.Label(x.history_window, text="ประวัติค่าใช้จ่าย")
        expenses_label.pack()
        for expense in x.ประวัติค่าใช้จ่าย:
            expense_item = tk.Label(x.history_window, text=f"ค่าใช้จ่าย: {expense} บาท")
            expense_item.pack()
